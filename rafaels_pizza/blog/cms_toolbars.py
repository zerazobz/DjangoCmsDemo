...
class PostToolbar(CMSToolbar):
    def populate(self):
        admin_menu = self.toolbar.get_or_create_menu("blog_menu", "Blog")
        ...
        post_menu.add_modal_item("Add New Post", url)
...
class CategoryToolbar(CMSToolbar):
    def populate(self):
        ...
        category_menu.add_modal_item("Edit Catefory", url=url)

        url = "/admin/blog/category/add"
        category_menu.add_modal_item("Add New Category", url)
